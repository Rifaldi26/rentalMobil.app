{{-- resources/views/users/notifikasi/index.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Notifikasi — Rental Mobil</title>
    ...
    @else
        @foreach($notifikasis as $notif)
            ...
            <form action="{{ route('notifikasi.baca', $notif) }}" method="POST" style="margin:0;">
                @csrf
                <button type="submit" class="notif-item {{ !$notif->dibaca ? 'belum-dibaca' : '' }}" style="width:100%;text-align:left;cursor:pointer;border:1px solid {{ !$notif->dibaca ? '#bfdbfe' : 'var(--gray-100)' }};">
                    @if(!$notif->dibaca)
